...
function color2str(c) {
	...
	if(v.length > 3)
		a = v[3]*255;
	return '#' + tohex(a) + tohex(v[0]) + tohex(v[1]) + tohex(v[2]);
}

var colors = [];
function addcolor(c) {
	var color = color2str(c);
...
function listcss() {
	if (document.styleSheets) {                            // If browser can play with stylesheets
		for (var i=0; i<document.styleSheets.length; i++) { 
			...
			if(styleSheet.href && styleSheet.href.indexOf("custom.css") < 0 ) {
				 continue;
			}
			...
			do {                                             
				...
				if (cssRule && cssRule.style)  {         
					if(cssRule.selectorText && cssRule.selectorText.substring(0, 3) == 'md-') {
						ii++;
						continue;
					}

					if(cssRule.style.color) {
						//console.log('Color: ' + cssRule.selectorText + '=' + addcolor(cssRule.style.color));                      
					}
					if(cssRule.style['background-color']) {
						console.log('BG Color: ' + cssRule.selectorText + '=' + addcolor(cssRule.style['background-color']));   
					}
				}                          
				ii++;                     
			} while (cssRule)                
	...
	for(var i = 0; i < colors.length; i++) {
		console.log("Unique colors: " + colors[i]);
	}	
}
